...
class RenderAlbum extends React.Component {
    ...
    render() {
        const randomAlbum = props.albumsData.filter((album) => album.id === Math.floor(Math.random() * 3) + 1);
        console.log(randomAlbum);

        return <div>{randomAlbum}</div>;
    }
}
    ...
    render() {
        ...
        return (
            <Switch>
                ...
                <Route exact path="/random" render={() => <RenderAlbum albumsData={this.state.albumsData} />} />
            </Switch>
        );
    }
}
